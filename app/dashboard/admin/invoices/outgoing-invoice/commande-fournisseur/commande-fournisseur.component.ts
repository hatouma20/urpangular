...
ngOnInit(): void {
  ...
  this.buillingService.getAllCommandeFournisseur().subscribe(resp => {
...
select() {
  ...
  this.filteredfournisseurList.next([]);
  // get facture number
  this.buillingService.getBonCommandeNumber().subscribe(response => {
...
getpayementmode(payementype: any) {
  ...
  editer(uuid: string) {
    ...
    this.uuidInvoice = uuid;
    document.querySelector('#collapseOne').classList.remove('show');
    document.querySelector('#collapseTwo').classList.add('show');
    const selectedInvoice = this.allInvoices.find(elt => elt.uuid === uuid);
    this.fournisseurService.gettiers(selectedInvoice.customer).subscribe( response => {
      this.nomfournisseur = response.name;
      this.codefournisseur = response.code;
      this.clientselect = response;
      this.coll = [];
      this.fournisseurAddressList.next([]);
      response.tiers_address.forEach(elt => {
        this.coll.push({item_id: elt.uuid, item_text: elt.location});
      });
      this.fournisseurAddressList.next(this.coll);
      const addressSelected = response.tiers_address.find(elt => elt.uuid === selectedInvoice.customerAdress);
      this.SelectAddress.item_text = addressSelected.location;
      this.SelectAddress.item_id = addressSelected.uuid;
      // this.select();
    });
    this.linestotallist = [];
    this.dateFrom = selectedInvoice.invoiceDate;
    this.dateFromlivraison = selectedInvoice.deliveryDate;
    this.dateFromeche = selectedInvoice.dueDate ;
    this.dateinvoice = this.dateFrom;
    this.dateLivraison = this.dateFromlivraison
    this.dateEchec = this.dateFromeche;
    this.facturelist = [];
    this.purchaseType = selectedInvoice.purchaseType;
    this.idInvoice = selectedInvoice.id;
    this.checkboxValue = selectedInvoice.tva;
    this.languetext = selectedInvoice.langue;
    this.uuidtier = selectedInvoice.customer;
    this.htmlContent = selectedInvoice.note;
    this.note = this.htmlContent;
    this.selectedCityName = selectedInvoice.model;
    this.deviseInvoice = selectedInvoice.devise;
    this.colorInvoice = selectedInvoice.color;
    this.adressedelect();
    const id = this.facturelist.length;
    selectedInvoice.lineModels.forEach(elt => {
      // tslint:disable-next-line:max-line-length
      if (elt.productname === 'line') {
        this.facturelist.push({type: 1, id, totalline: ''});
      } else if (elt.productname === 'subtotal') {
        this.facturelist.push({type: 2, id, totalline: ''});
      } else {
        this.facturelist.push({type: 3, id, totalline: ''});
      }
      this.linestotallist.push(
        new LineModel(
          elt.id,
          elt.productname,
          elt.descriptionproduct,
          elt.quantite,
          elt.prixunitaire,
          elt.prixtotal,
          elt.tva,
  ...
  TransfererBRecept(id: number) {
    const BonReceptModel = {
      ...
      uuid: '',
      id: null,
      purshaseOrder: this.allInvoices[id].uuid,
      has_invoice: this.allInvoices[id].has_invoice,
      has_receipt: true
    };
    this.fineshedgenerateBR.next(false);
    this.buillingService.getBonReceptionNumber().subscribe(resp => {
      BonReceptModel.invoiceNumber = resp.number;
      this.fineshedgenerateBR.next(true);
    });
    this.fineshedgenerateBR.subscribe(value => {
      if (value) {
        this.buillingService.createBonRecept(BonReceptModel).subscribe(response => {
            // this.ref.detectChanges();
