...
@Component({
  ...
  selector: 'wind-modele1',
  templateUrl: './modele1.component.html',
  styleUrls: ['modele1.component.css'],
  styles: [
    ` ::ng-deep @media print {
          ::ng-deep .page {
...
export class Modele1Component implements OnInit {
  ...
  getSumtva() {
    ...
    if (this.devise === '$') {
      ...
      this.totalLetter = wordentnd + ' Euros et ' + wordenmil + ' Centimes ';
    }  else {
  ...
  changeshowlist() {
    ...
    const payementheight = document.getElementById('payement').scrollHeight;
    const footerheight = document.getElementById('footer').scrollHeight;

    const totalsHeight = tabletotalheight + noteheight + sommelettreheight + payementheight;
